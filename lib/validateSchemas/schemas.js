...
exports.validateSchemaForReadAReadRequest = Joi.string().regex(/^[A-Z0-9]/i).min(5).max(30).required();
exports.validateSchemaForComment = Joi.string().min(4).max(50).required();
exports.validateSchemaForName = Joi.string().regex(/\D/).min(3).max(10).required();
...
exports.validateSchemaForNewBook = {
    name: Joi.string().regex(/^[A-Z0-9]/i).min(1).max(30).required(),
    author: Joi.string().regex(/^[A-Z]/i).min(2).max(20).required(),
    description: Joi.string().regex(/^[A-Z]/i).min(3).max(100).required(),

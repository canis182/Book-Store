...
module.exports = class Comment {
  static async searchComment(name) {
    try {
      ...
      const resultSearchComments = await Comments.find({ user: userNickname.nickname }, { __v: 0 });
  ...
  static async delateAllCommentByUserName(name) {
    try {
      ...
      for (let i = 0; i < allUserComments.length; i++) {
        const idBook = allUserComments[i];
        for (const key in idBook) {
          new Promise((resolve) => {
            resolve(Book.update(
              { _id: idBook[key] },
            ...
            .then(() => Comments.remove({ book: idBook[key], user: userNickname.nickname }));
        }
      }
      return {
